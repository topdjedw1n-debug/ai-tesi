...
test.describe('Magic Link Authentication', () => {
  test('should complete full authentication flow from login to dashboard', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Thesica/i)
    ...
    // Step 3: Click "Send Magic Link" button
    ...
    // Step 4: Simulate clicking magic link (navigate to verify page with token)
    ...
    // Step 5: System should redirect to dashboard after successful verification
  ...
  test('should enforce rate limiting on magic link requests', async ({ page }) => {
    // Setup: Mock first request to succeed
    await mockApiRoute(page, '**/api/v1/auth/magic-link', {
