...
os.environ.setdefault("DATABASE_URL", "sqlite+aiosqlite:///./test.db")  # Use same DB as other tests
...
async def test_duplicate_webhook_ignored_by_status_check(
    ...
    print(f"✅ Test 2 PASSED: Duplicate webhook ignored via status check (idempotent)")
...
async def test_payment_webhook_creates_generation_job_once(
    ...
    print(f"✅ Test 4 PASSED: Webhook created job once, duplicate skipped")
...
async def test_stripe_signature_validation_rejects_invalid(
    ...
    print(f"✅ Test 5 PASSED: Invalid signature rejected with 400")
...
async def test_payment_intent_preserves_metadata(
    ...
    print(f"✅ Test 6 PASSED: Metadata preserved in Stripe and DB")
...
async def test_webhook_event_type_logged_for_debugging(
    ...
    with patch('app.services.payment_service.logger') as mock_logger, \
         patch('app.api.v1.endpoints.payment.BackgroundJobService'):
    ...
    print(f"✅ Test 7 PASSED: Webhook event type logged for debugging")
...
async def test_race_condition_webhook_job_creation_prevented(
    ...
    with patch('app.api.v1.endpoints.payment.BackgroundJobService'):
        ...
        jobs_list = jobs.scalars().all()
        assert len(jobs_list) == 1, f"Expected 1 job, got {len(jobs_list)} (SELECT FOR UPDATE failed)"
        
        # VERIFY: Job has correct status
        assert jobs_list[0].status in ["queued", "running"]
        assert jobs_list[0].user_id == test_user.id
    
    print(f"✅ Test 8 PASSED: Race condition prevented by SELECT FOR UPDATE")
