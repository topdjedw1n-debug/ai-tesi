...
async def auth_token(client: AsyncClient, test_user: User):
    ...
    response = await client.post(
        "/api/v1/auth/verify-magic-link", json={"token": "test_magic_link_webhook_e2e"}
    )
...
async def other_auth_token(client: AsyncClient, other_user: User):
    ...
    response = await client.post(
        "/api/v1/auth/verify-magic-link", json={"token": "test_magic_link_other_user"}
    )
    assert response.status_code == 200
    return response.json()["access_token"]
...
async def test_webhook_payment_intent_failed_e2e(
    ...
    # Create webhook event
    event = webhook_event_factory("payment_intent.payment_failed", "pi_test_failed_e2e")
    mock_stripe.return_value = event
    ...
    print("✅ Test 2 PASSED: payment_intent.payment_failed handled correctly")
...
async def test_webhook_payment_intent_canceled_e2e(
    db_session: AsyncSession, test_user: User, mock_stripe, webhook_event_factory
):
    """
    ...
    # Create webhook event
    event = webhook_event_factory("payment_intent.canceled", "pi_test_canceled_e2e")
    mock_stripe.return_value = event
    ...
    print("✅ Test 3 PASSED: payment_intent.canceled handled correctly")
...
@pytest.mark.asyncio
async def test_webhook_unhandled_event_logged(db_session: AsyncSession, mock_stripe):
    """
    ...
    print("✅ Test 4 PASSED: Unhandled event logged correctly")
...
async def test_verify_payment_endpoint_success(
    ...
    response = await client.get(
        "/api/v1/payment/verify?session_id=cs_test_verify_e2e",
    ...
    print("✅ Test 5 PASSED: Verify payment endpoint works correctly")
...
@pytest.mark.asyncio
async def test_verify_payment_not_found(client: AsyncClient, auth_token: str):
    """
    ...
    print("✅ Test 6 PASSED: Verify payment not found returns 404")
...
async def test_verify_payment_ownership_check(
    ...
    print("✅ Test 7 PASSED: IDOR protection works (404 for other user)")
...
async def test_webhook_payment_not_found_error(
    db_session: AsyncSession, mock_stripe, webhook_event_factory
):
    """
    ...
    event = webhook_event_factory("payment_intent.succeeded", "pi_nonexistent_payment")
    mock_stripe.return_value = event
    ...
    print("✅ Test 8 PASSED: Missing payment raises ValueError")
