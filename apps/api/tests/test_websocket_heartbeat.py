...
class TestWebSocketHeartbeat:
    ...
    async def test_heartbeat_sends_messages_periodically(self):
        ...
        # Patch dependencies
        with patch('app.services.background_jobs.database.AsyncSessionLocal', return_value=AsyncMock(__aenter__=AsyncMock(return_value=mock_db), __aexit__=AsyncMock())):
    ...
    async def test_heartbeat_stops_when_job_fails(self):
        ...
        mock_job = AIGenerationJob(
            id=2,
            document_id=200,
            status="failed",  # Job already failed
            progress=50
        )
