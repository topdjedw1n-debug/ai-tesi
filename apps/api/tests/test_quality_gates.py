...
def mock_document():
    ...
    doc.outline = {
        "sections": [
            {"title": "Introduction", "word_count_target": 500}
        ]
    }
    return doc
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            mock_ai.return_value = (
                                45.0, "humanized", "gptzero", True, None
                            )
                            ...
                            with patch(
                                ...
                                with patch(
                                    ...
                                    with pytest.raises(QualityThresholdNotMetError) as exc_info:
                                        ...
                                        for attempt in range(3):
                                            ...
                                            # Check quality
                                            grammar_ok = await mock_grammar(content, "en", 10)
                                            plagiarism_ok = await mock_plagiarism(content, 85.0)
                                    ...
                                    assert "Plagiarism" in str(exc_info.value) or "quality" in str(exc_info.value).lower()
                                    
                                    # Verify regeneration attempts were made
                                    assert mock_gen.generate_section.call_count == 3  # 3 attempts
...
async def test_quality_gate_allows_good_content(
    ...
    with patch("app.services.background_jobs.settings", mock_settings_gates_enabled):
        with patch("app.services.background_jobs.SectionGenerator") as MockGenerator:
            ...
            mock_gen.generate_section = AsyncMock(
                return_value={
                    "content": "High quality test content",
                    "citations": []
                }
            )
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            mock_ai.return_value = (
                                45.0, "humanized content", "gptzero", True, None
                            )
                            ...
                            for attempt in range(3):
                                ...
                                ai_result = await mock_ai(content, 55.0, mock_humanizer, "openai", "gpt-4")
            ...
            with patch("app.services.background_jobs.Humanizer") as MockHumanizer:
                ...
                with patch(
                    ...
                    mock_grammar.return_value = (
                        50.0, 50, False, "Too many grammar errors"  # FAIL ❌
                    )
                    ...
                    with patch(
                        ...
                        mock_plagiarism.return_value = (
                            40.0, 60.0, False, "Plagiarism too high"  # FAIL ❌
                        )
                        ...
                        with patch(
                            ...
                            mock_ai.return_value = (
                                80.0, "content", "gptzero", False, "AI score too high"  # FAIL ❌
                            )
                            ...
                            assert mock_gen.generate_section.call_count == 1  # Only 1 attempt
