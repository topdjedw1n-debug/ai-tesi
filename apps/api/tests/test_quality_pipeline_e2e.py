...
class TestQualityValidatorE2E:
    ...
    async def test_citation_density_low_citations(self):
        ...
        """ + " word " * 490  # Total ~500 words
        
        result = await validator._check_citation_density(content)
        
        assert result["score"] < 100, "Score should be < 100 for low citations"
        assert result["details"]["total_citations"] == 1, f"Expected 1 citation, found {result['details']['total_citations']}"
    ...
    async def test_citation_density_good_citations(self):
        ...
        assert result["score"] == 100.0, f"Score should be 100 for sufficient citations, got {result['score']}"
        assert result["details"]["total_citations"] >= 3, f"Expected >=3 citations, found {result['details']['total_citations']}"
        assert len(result["issues"]) == 0
    ...
    async def test_academic_tone_excessive_first_person(self):
        ...
        """ + " word " * 20  # ~50 words total, 15 first-person = 30%
        
        result = await validator._check_academic_tone(content)
        
        assert result["score"] < 100, "Score should be penalized for excessive first-person"
...
Moreover, these findings have important practical applications.
    ...
    async def test_validate_section_with_error_handling(self):
        ...
        assert result["passed"] is True, "Should not fail generation on validation error"
