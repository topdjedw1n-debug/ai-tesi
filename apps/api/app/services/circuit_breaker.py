        ...
        async with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                if self._should_attempt_reset():
                    ...
                    wait_seconds = self.recovery_timeout - (datetime.now() - self.last_failure_time).total_seconds()  # type: ignore
                    logger.warning(f"Circuit breaker is OPEN. Waiting {wait_seconds:.1f}s before retry")
