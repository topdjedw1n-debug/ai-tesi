...
class BackgroundJobService:
    ...
    async def generate_full_document(
        ...
        async with database.AsyncSessionLocal() as db:
            try:
                logger.info(f"Starting full document generation for document {document_id}")
                ...
                if not document.outline:
                    ...
                    try:
                        ...
                        logger.info(f"Outline generated successfully for document {document_id}")
                    except Exception as e:
                ...
                if not document.outline or "sections" not in document.outline:
                    logger.error(f"No outline sections found for document {document_id}")
                    await db.execute(
                        update(Document)
                        .where(Document.id == document_id)
                        .values(status="failed")
                ...
                logger.info(f"Generating {len(sections)} sections for document {document_id}")
                ...
                for idx, section_data in enumerate(sections):
                    ...
                    try:
                        ...
                        ] if context_sections else None

                        logger.info(f"Generating section {section_index}: {section_title}")
                        ...
                        logger.info(f"Humanizing section {section_index}: {section_title}")
                ...
                try:
                    ...
                    logger.info(f"Document {document_id} exported successfully: {export_result.get('download_url')}")
                except Exception as e:
                ...
                logger.error(f"Critical error in background document generation: {e}", exc_info=True)
    ...
    async def process_custom_requirement(
        ...
        async with database.AsyncSessionLocal() as db:
            try:
                logger.info(f"Processing custom requirement for document {document_id}: {file_path}")
                ...
                if file_path.endswith(".pdf"):
                    extracted_text = await BackgroundJobService._extract_pdf_text(file_path)
                elif file_path.endswith((".doc", ".docx")):
                    extracted_text = await BackgroundJobService._extract_docx_text(file_path)
                else:
