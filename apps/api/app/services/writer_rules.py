...
Cite supplied evidence with exact [KEY] markers. Quote verbatim only from page-labelled excerpts and append p. N after [KEY] using only supplied page numbers; at most one quotation per paragraph, under twenty words, and quoted words stay below one tenth of the section: the anchor is normally the fact, the number, the article or the decision in your own words, not a quotation. A source supplied as an abstract is paraphrased and cited without quotation marks. Cite statutes by article and comma (append art. N, comma N after [KEY]) and judgments by their decision, never by PDF page. For a statute or an authority's act: name the article and state its effect for the section's question in at most one paragraph, with the conditions and exceptions that matter here; do not walk through the act clause by clause, and quote its operative words only where that wording itself is analysed, within the quotation budget; when an act already treated in an earlier section returns, refer back to it by article and add only what this section needs. A judgment is told as its facts, the question decided and the principle stated, with the decision as anchor. When the section is about a concept, a role, a principle or a framework, build it from the specific content of one primary document (its articles with their numbers, its data, its case), not from a general explanation of the concept.
...
CONCLUSIONS_RULE = (
    FRAME_RULE
    + """Answer the research question with the facts in findings, each carried by its [KEY] and locator, grouped by the logic of the answer; then name two limits in plain sentences. No new sources, no outlook paragraph, and no walk through the chapters: never write "il primo/secondo/terzo capitolo", "si articola" or any sentence that names or numbers a chapter.
"""
)
