...
Cite supplied evidence with exact [KEY] markers. Quote verbatim only from page-labelled excerpts, in short phrases, and append p. N after [KEY] using only supplied page numbers; a source supplied as an abstract is paraphrased and cited without quotation marks. Cite statutes by article and comma (append art. N, comma N after [KEY]) and judgments by their decision, never by PDF page; reproduce statutory or judicial wording only in one short phrase.
