...
async def list_users(
    ...
    try:
        ...
        log_security_audit_event(
            ...
            action="list",
            outcome="success",
            details={"page": page, "per_page": per_page, "has_search": search is not None}
        )
...
async def verify_backup(
    ...
    try:
        ...
        log_security_audit_event(
            ...
            outcome="success" if result.get("status") == "healthy" else "needs_attention",
            details={"status": result.get("status"), "checks": result.get("checks", {})}
        )
...
async def verify_storage(
    ...
    try:
        ...
        status = "healthy" if result.get("missing_files", 0) == 0 and result.get("orphaned_files", 0) == 0 else "needs_attention"
