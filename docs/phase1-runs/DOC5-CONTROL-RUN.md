...
- [ ] **Source pack**: `document_sources` має рядки з `citation_key`,
      `is_in_upfront_pack=true`, `on_topic_score >= 0.35`.
- [ ] **Grounding gate**: події в провенансі; НЕМАЄ хибних
      `no concrete evidence`-фейлів на секціях із названими системами (B4.1).
...
- [ ] **Claim-audit**: `claim_check_summary` подія, `checked <= 50`,
      `DocumentSection.claim_verification` заповнено (B5).
...
- [ ] **Негативний тест (B1)**: окремий короткий прогін із вимкненим
      AI-детектором (прибрати GPTZero-ключ) → у QA Evidence
