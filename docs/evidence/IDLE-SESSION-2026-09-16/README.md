...
## Доставка пунктів 1–4 (фаундер, 16.09 ≈ 16:55 UTC+3: «команди виконав»)

Дві команди фаундера (`rsync -ai --relative apps/api/app apps/api/tests apps/web thesica:/opt/thesica/`, `deploy.sh` — збирає образи api і web, перезапускає обидва). Звірка після «готово»:

- `ai-thesis-api`: 9/9 змінених файлів застосунку збігаються за sha256 із локальним `5344abe` (jobs.py, schemas/document.py, rag_retriever.py, citation_render.py, executor_v2/assemble.py, executor_v2/sources.py, generation_warnings.py, placeholder_notes.py, production_case_service.py); у контейнері імпортуються нові модулі, `legal_citation` віддає форму «come modificato dall'art. 23, …». Два скрипти (`scripts/editor_note.py`, `scripts/fabrication_check.py`) у доставку не входять — команда не охоплює `apps/api/scripts`; записка редактору запускається локально на записах, серверу не потрібна.
- `ai-thesis-web`: три змінені джерела збігаються з локальними; зібраний бандл у контейнері містить текст підказки («Розділ буде написано з того, що є в пакеті», 8 збігів у `.next`).
- Контейнери `ai-thesis-api` і `ai-thesis-web` перезапущені й `healthy`; postgres/redis/minio не чіпалися.

Перше живе замовлення йде з цього стану сервера (`5344abe`).

## Пункт 5 — операційна репетиція без провайдерів (протокол)

Без нових інструментів і без живих викликів: тест виконавця та наявні тести API/кабінету, запущені 16.09 на `5344abe` + тест нижче.

**(а) Скасування → наступний етап не стартує.** `tests/test_executor_v2.py::test_cancellation_during_a_stage_does_not_start_the_next_one[S2|S3]` — **2 passed**. Механізм: `cancel_active_generation_job` знімає lease (статус `cancelled`, токен очищено); кожен наступний огороджений запис виконавця (стан етапу, пакет, план, розділ, DOCX) втрачає lease і кидає `GenerationLeaseLostError`, тож `step()` наступного етапу не доходить до виклику моделі. Факти з тесту: скасування під час S2 → 1 виклик моделі (S1), кроки `S1, S2`, S3 не стартує; скасування під час виклику S3 → 2 виклики (S1, S3 — уже відправлений виклик завершується і коштує), кроки `S1, S2, S3`, S4 не стартує; в обох гілках `run()` повертає `None` без технічної зупинки (`stop = None`, `error_message = None`), запуск і робота лишаються `cancelled`, розділів 0, DOCX немає, є подія `generation_cancelled`. Межа: між скасуванням і наступним огородженим записом виконавець може чекати на вже відправлену відповідь моделі (до `provider_timeout_seconds`); heartbeat помічає втрату lease не пізніше ніж за 15 с.

**(б) Завершена робота → завантаження → запис рішення.** Наявні тести на завершеному документі (CLI replay кнопок не перевіряє, тому доказ — тести API та інтерфейсу):

| Крок ланцюжка | Що доводить | Тести (усі PASS) |
|---|---|---|
| Завершена робота видима менеджеру; статус останнього запуску переживає перезавантаження; робота веде на свою справу | `test_manager_workflow.py` (8) — статус запуску після reload без чужих робіт; документ → справа; «released» = файл готовий менеджеру, не видача клієнту; рецензія DOCX прив'язана до артефакту; приймання вимагає дійсної кількості сторінок | 8 |
| Завантаження: «DOCX для Compilatio» / експорт видає токен на вже зібраний файл без перегенерації; захищене завантаження відхиляє підмінені байти й стару версію, відповідь приватна | `test_document_download.py` (8) | 8 |
| Запис рішення: результат детектора структурований, прив'язаний до sha256 поточного артефакта, відхиляє файл, підмінений під тим самим шляхом; видача вимагає доказів і обох рішень детектора на тому самому артефакті; ручне «PASS» не приймає 22 %; справа віддає поточні попередження без сирих записів | `test_production_cases.py` (7 обраних) | 7 |
| Скасування лишається термінальним і обліковує витрати в польоті | `test_generation_cancel.py` (7) | 7 |
| Кабінет: сторінка видачі показує «DOCX для Compilatio» зі sha256, тримає незбережені докази лише для того самого DOCX, блокує «Дозволити видачу» без доказів; форми доказів вимагають звіт і явне підтвердження точного файла, фіксують перевищення без ручного прийняття; сторінка роботи показує попередження текстом | `detail.test.tsx`, `ReleaseEvidenceForms.test.tsx`, `documents/[id]/page.test.tsx` — **25 passed** | 25 |

Разом: API 30 passed (лог `point5b-api.log` у scratchpad сесії), web 25 passed. **Не перевірено:** клік у живому кабінеті на сервері (немає тестового акаунта в сесії), реальне завантаження звіту Compilatio, замовлення з понад 30 сторінками.

## Пункт 6 — канон

`docs/AGENT_SYNC.md`, блок `founder-priority-2026-09-09`: доданий «Стан 16.09.2026» (заморожений писар і перелік того, що не робимо; що показали скани — економіка 3/3 один файл без людського читання, similarity права ≈ 15–17 % як питання політики; режим першого замовлення з ролями, без гейта $0,10, скасування лише порожнього/чужого пакета, рівні 15/14 → 10/10, налаштування цитат Compilatio; межі 20–30 сторінок і стеля $5 під конкретне замовлення; один наступний крок). `THESICA-PLAN.md`: один абзац угорі з тим самим наступним кроком і посиланнями; записи нижче позначені як історія. Перевірка: новий виконавець, що читає CLAUDE.md → AGENT_SYNC → THESICA-PLAN, отримує один крок — перше живе замовлення на `5344abe` — і перелік дослідів, які не запускати.
