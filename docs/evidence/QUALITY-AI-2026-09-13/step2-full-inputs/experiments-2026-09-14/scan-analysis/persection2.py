...
def sections(lines_path, titles):
    ...
    while i<len(L):
        ...
        if matched:
            sec=matched[0]; out.setdefault(sec,{"words":0,"ai":0,"quotes":0,"text":[]}); i+=matched[1]; continue
        l=L[i]; i+=1
        if sec is None: continue
        if l["words"]<=2 and NUMERIC.match(l["text"].replace("‎","")): continue
        if "Punti di interesse" in l["text"]: continue
        o=out[sec]; o["words"]+=l["words"]; o["ai"]+=sum(l["ai_flags"]); o["quotes"]+=sum(l["quotes_flags"]); o["text"].append(l["text"])
    for o in out.values():
        o["ai_pct"]=round(100*o["ai"]/o["words"],1) if o["words"] else None
        o["quotes_pct"]=round(100*o["quotes"]/o["words"],1) if o["words"] else None
    return out
if __name__=="__main__":
    ...
    for work,fix in (("A","fix-A"),("B","fix-B-v1")):
        ...
        print(f"{'#':>3} {'section':38s} {'old AI':>7} {'new AI':>7} {'dAI':>6} {'old quo':>8} {'new quo':>8} {'words o/n':>10} text_ratio")
        rows=[]
        for idx,t in enumerate(titles+["Bibliografia","Normativa e giurisprudenza"],1):
            ...
            print(f"{idx:>2}{'*' if idx in live else ' '} {t[:38]:38s} {str(oa):>7} {str(na):>7} {('%+.1f'%d) if d is not None else '':>6} {str(o['quotes_pct'] if o else None):>8} {str(n['quotes_pct'] if n else None):>8} {str(o['words'] if o else 0)+'/'+str(n['words'] if n else 0):>10} {ratio}")
        ...
        print(f" unchanged {len(un)} sections: AI old {agg(un,'old','ai')} -> new {agg(un,'new','ai')} | quotes old {agg(un,'old','quotes')} -> new {agg(un,'new','quotes')}")
        print(f" rewritten sections: AI old {agg(lv,'old','ai')} -> new {agg(lv,'new','ai')} | quotes old {agg(lv,'old','quotes')} -> new {agg(lv,'new','quotes')}")
        al=[r for r in rows]; print(f" all mapped words old {sum(r['old']['words'] for r in al if r['old'])} new {sum(r['new']['words'] for r in al if r['new'])}: AI old {agg(al,'old','ai')} -> new {agg(al,'new','ai')} | quotes old {agg(al,'old','quotes')} -> new {agg(al,'new','quotes')}")
