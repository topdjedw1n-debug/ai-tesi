...
## 7. Доповнення 13.09 (вечір): перевизначення на рівні окремого звернення

Після досліду на B консультанти (Astra, Grok) порадили тестувати Opus 5 з явно вимкненим thinking і спершу робити дешевий smoke одного розділу на новій моделі. Для цього лабораторію доопрацьовано без зміни виробничого коду:

- `--model` тепер підставляється **на кожне живе звернення письма** (S4/S6), а не в `ctx.model` для всіх розділів: у `live` розділи, взяті зі стрічки, зберігають записану модель і запит, тому `--live-sections 5 --model claude-opus-5` пише лише розділ 5 новою моделлю. В `exact` перевизначення застосовується до всіх звернень S4/S6, щоб запис варіанта відтворювався так, як був зроблений (а чужий запис зупинявся, як і раніше).
- `--request-override JSON` додає параметри до запитів письма (наприклад `{"thinking": {"type": "disabled"}}`); `messages`, `max_tokens` і `model` замінювати не можна. Фактичний запит із доданими параметрами записується у квитанцію штатним журналом.
- Перевірено офлайн: exact на job16 без перевизначень — той самий DOCX `08e203b5…`; exact із `--model claude-opus-5` — зупинка «Model differs at S4»; сухий прогін `--mode live --live-sections 5 --model claude-opus-5 --request-override '{"thinking":{"type":"disabled"}}'` з недійсним ключем — розділи 1–4 зі стрічки (модель `claude-opus-4-8`, без thinking), розділ 5 — живе звернення до `claude-opus-5` з `thinking: disabled` і штатною зупинкою 401, витрат 0. Тести лабораторії: 6 passed.
