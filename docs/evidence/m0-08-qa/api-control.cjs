...
const state=JSON.parse(fs.readFileSync(path.join(__dirname,'browser-state.json')));
const token=state.origins.find(o=>o.origin==='http://localhost:3120').localStorage.find(o=>o.name==='auth_token').value;
async function api(url,method='GET',data){
 const r=await fetch('http://localhost:8320/api/v1'+url,{method,headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:data?JSON.stringify(data):undefined});
...
function sql(query){return execFileSync('docker',['exec','thesica-resilience-postgres-1','psql','-U','resilience','-d','resilience','-At','-c',query],{encoding:'utf8'}).trim()}
