...
async def main():
 out={'origin':'live-cabinet-document12-job13-executor-v2-6e432a4','exported_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'revision':'6e432a497fae8afbece0eb49834e127e00725702','document_id':12,'job_id':13}
 async with database.AsyncSessionLocal() as db:
  ...
  for table,condition in [('documents','id=12'),('ai_generation_jobs','document_id=12'),('document_sections','document_id=12'),('document_sources','document_id=12'),('document_source_files','document_id=12'),('document_provenance','document_id=12'),('production_cases','document_id=12')]:
   ...
   out[table]=rows
  jobs=out['ai_generation_jobs'];assert len(jobs)==1 and jobs[0]['id']==13 and jobs[0]['request_payload']['executor_version']==2
  assert jobs[0]['status'] in ('completed','failed','cancelled'),'Not terminal yet'
  usage,unknown=await journal_usage(db,12,13)
