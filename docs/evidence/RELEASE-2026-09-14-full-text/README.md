...
> **Оновлення 14.09, вечір:** до релізного пакета додано пакет «оформлення» (S5/S6, `citation_render.py`, `docx_export.py`, поле змісту в експорті). Після коміту код повторно доставлено на сервер тим самим `rsync`; реліз (`deploy.sh`) і акаунт — за фаундером, як описано вище.
